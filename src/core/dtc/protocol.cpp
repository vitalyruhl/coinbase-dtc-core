...
std::vector<uint8_t> LogonRequest::serialize() const {
    ...
    memcpy(buffer.data() + offset, username.c_str(), sizeof(username));
    offset += sizeof(username);
    memcpy(buffer.data() + offset, password.c_str(), sizeof(password));
    offset += sizeof(password);
    memcpy(buffer.data() + offset, general_text_data.c_str(), sizeof(general_text_data));
